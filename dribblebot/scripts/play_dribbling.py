...
def load_env(method, load_run, checkpoint, headless, device):
    if method == "mlp":
      ...
      experiment_name = "dribbling"
    elif method == "aug":
      from dribblebot_learn.ppo_aug import Runner
      from dribblebot_learn.ppo_aug.actor_critic import AC_Args
      from dribblebot_learn.ppo_aug import RunnerArgs
      experiment_name = "dribbling_aug"
    elif method == "emlp":
      from dribblebot_learn.ppo_symm import Runner
      from dribblebot_learn.ppo_symm.actor_critic import AC_Args
      from dribblebot_learn.ppo_symm import RunnerArgs
      experiment_name = "dribbling_emlp"
    else:
    ...
    # RunnerArgs.resume_path = f"{experiment_name}/runs/{run_name}"
    RunnerArgs.resume_checkpoint = f'tmp/{method}/{load_run}/ac_weights_{checkpoint}.pt' 

    Cfg.robot.name = "go1"
    Cfg.sensors.sensor_names = [
                        "ObjectSensor",
                        "OrientationSensor",
                        "ProjectedForwardSensor",
                        "RCSensor",
                        "JointPositionSensor",
                        "JointVelocitySensor",
                        "ActionSensor",
...
def play_go1(args):
    ...
    env, policy = load_env(method, load_run, checkpoint, headless=headless, device=device)
    ...
    import imageio
    mp4_writer = imageio.get_writer(f'dribbling.mp4', fps=50)
    ...
    for i in tqdm(range(num_eval_steps)):
        ...
        obs, rew, done, info = env.step(actions.detach())
